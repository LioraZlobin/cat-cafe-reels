...
private isFreeSpinAutoPlay = false;
...
private readonly activePaylineTweens:
  ...
  private historyButton!: Phaser.GameObjects.Rectangle;
  private historyContainer?: Phaser.GameObjects.Container;
  private historyMaskGraphics?: Phaser.GameObjects.Graphics;
  private readonly spinHistory: SpinHistoryEntry[] = [];
...
create(): void {
  ...
  /*
   * הדפדפן מאפשר להתחיל מוזיקה רק לאחר
   * האינטראקציה הראשונה של המשתמש.
   */
  this.input.once(
    "pointerdown",
    () => {
...
private createUtilityButtons(): void {
  ...
  drawHudButton(buttonX, soundY);
  drawHudButton(buttonX, historyY);
  ...
  const historyButtonText = this.add
    .text(
      buttonX,
      historyY,
      "ROUND\nHISTORY",
  ...
  this.soundButton.on(
    ...
    () => {
      ...
      this.soundManager.setEnabled(
        enabled,
      );

      this.soundButtonText.setText(
        enabled
          ? "♪\nSOUND"
          : "×\nMUTED",
      );
  ...
  historyButtonText.setInteractive({
    useHandCursor: true,
  });

  historyButtonText.on(
    "pointerdown",
    () => {
      this.soundManager.playButtonClick();
      this.toggleHistory();
...
private showHistory(): void {
  ...
  const drawPawWatermark = (
    ...
    panel.fillStyle(0xffd85c, 0.075);
  ...
  const headers = [
    this.add.text(-305, -190, "#", headerStyle),
    this.add.text(-205, -190, "TYPE", headerStyle)
      .setOrigin(0.5, 0),
    this.add.text(80, -190, "BET", headerStyle)
      .setOrigin(1, 0),
    this.add.text(295, -190, "WIN", headerStyle)
      .setOrigin(1, 0),
  ];
  ...
  if (this.spinHistory.length === 0) {
    ...
    const rowsContainer = this.add.container(
      0,
      -145,
    );

    const rowHeight = 42;
    const viewportHeight = 370;
    const contentHeight =
      this.spinHistory.length * rowHeight;
    ...
    /*
     * כפות שקופות בשכבה שמעל השורות,
     * כדי שיישארו נראות גם כשההיסטוריה מלאה.
     */
    const pawOverlay = this.add.graphics();

    const drawOverlayPaw = (
      x: number,
      y: number,
      scale: number,
    ): void => {
      pawOverlay.fillStyle(
        0xffd85c,
        0.11,
      );

      pawOverlay.fillEllipse(
        x,
        y + 9 * scale,
        30 * scale,
        23 * scale,
    ...
    drawOverlayPaw(-315, -95, 0.8);
    drawOverlayPaw(315, 55, 0.8);
    drawOverlayPaw(-315, 190, 0.65);
    drawOverlayPaw(315, -175, 0.65);
    ...
    const scrollHint = this.add
      .text(
        ...
        248,
        maxScroll > 0
          ? "↕  Scroll or drag to see more rounds"
...
private closeHistory(): void {
  ...
  this.historyMaskGraphics = undefined;
  this.historyContainer?.destroy(true);
...
private showFreeSpinsIntro(
  ...
  this.showTemporaryMessage(
    "Press SPIN or AUTO PLAY for Free Spins",
  );
},
      });
    },
...
this.updateGameInfo();
  ...
  /*
   * שומרים כל סיבוב שהסתיים בהיסטוריה.
   * סוג הסיבוב נשמר לפני איפוס currentSpinIsFree.
   */
  this.addSpinToHistory(
...
this.playBoardWinGlow(
  ...
  if (this.isFreeSpinAutoPlay) {
    ...
    /*
     * במצב ידני מחכים ללחיצה על Spin.
     */
    this.setSpinButtonEnabled(true);
    this.updateBetControls();
  }
